...
from sleeper_tooling.cli import app, filter_exact_position, parse_positions, resolve_season_week
...
def test_resolve_season_week_defaults_season_to_current_year(monkeypatch) -> None:
    fake_client = FakeSleeperClient()
    monkeypatch.setattr(cli, "current_season_year", lambda: 2030)

    assert resolve_season_week(fake_client, None, 4) == (2030, 4)
    assert fake_client.calls == []


def test_resolve_season_week_uses_state_only_for_missing_week(monkeypatch) -> None:
    fake_client = FakeSleeperClient()
    monkeypatch.setattr(cli, "current_season_year", lambda: 2030)

    assert resolve_season_week(fake_client, None, None) == (2030, 1)
    assert fake_client.calls == [("state",)]
...
def test_configure_context_resolves_team_and_writes_env_file(monkeypatch, tmp_path) -> None:
    fake_client = FakeSleeperClient()
    monkeypatch.setattr(cli, "SleeperClient", lambda **_: fake_client)
    env_file = tmp_path / "sleeper-mcp.env"
    ...
    result = runner.invoke(
        ...
        [
            "--no-cache",
            "configure-context",
            "https://sleeper.com/leagues/1389328071634460672/matchup",
            "--team",
            "Mile High",
            "--env-file",
            str(env_file),
    ...
    assert result.exit_code == 0
    data = json.loads(result.stdout)
    assert data["league_id"] == "1389328071634460672"
    assert data["roster_id"] == 1
    assert data["owner_id"] == "u1"
    assert data["env"]["SLEEPER_DEFAULT_ROSTER_ID"] == "1"
    assert data["written"] is True
    assert data["env_file"] == str(env_file)
    assert env_file.read_text(encoding="utf-8") == (
        "# Generated by sleeper configure-context.\n"
        'SLEEPER_DEFAULT_LEAGUE_URL="https://sleeper.com/leagues/1389328071634460672/matchup"\n'
        "SLEEPER_DEFAULT_LEAGUE_ID=1389328071634460672\n"
        "SLEEPER_DEFAULT_ROSTER_ID=1\n"
        "SLEEPER_DEFAULT_OWNER_ID=u1\n"
        'SLEEPER_DEFAULT_TEAM_NAME="Mile High"\n'
        'SLEEPER_DEFAULT_DISPLAY_NAME="Neil"\n'
    )
    assert fake_client.calls == [
        ("users", "1389328071634460672"),
        ("rosters", "1389328071634460672"),
    ]


def test_configure_context_can_print_env_without_writing(monkeypatch, tmp_path) -> None:
    fake_client = FakeSleeperClient()
    monkeypatch.setattr(cli, "SleeperClient", lambda **_: fake_client)
    env_file = tmp_path / "sleeper-mcp.env"
    ...
    assert result.exit_code == 0
    assert "SLEEPER_DEFAULT_LEAGUE_ID=1389328071634460672" in result.stdout
    assert "SLEEPER_DEFAULT_ROSTER_ID=1" in result.stdout
