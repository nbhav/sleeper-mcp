...
- Separate raw `api_cache` response caching from the normalized decision database used for trendable reads.
...
- Treat normalized decision-data sync and trend reads as the Python/SQLite analytical layer. Worker D1 parity is planned separately.
...
For trendable decision reads, first call `decision_data_status`. If normalized
data is missing or stale, call `sync_decision_data`, then prefer
`player_stat_trends` or `position_stat_leaders` for week-over-week analysis.
...
Normalized decision-data sync and freshness checks:
...
`api_cache` is only a raw HTTP response cache. The normalized decision
...
The retention default is two seasons: current season plus one previous
